                    ...
                    <tr>
                      ...
                      <br>
                        ...
                        <p style="margin: 0;">
                            ...
                            @elseif($type == 1)
                                kindly click <a href="{{ env('APP_URL') }}/user/verify/{{base64_encode($user->email)}}/{{base64_encode($user->verification_code)}}">here</a> to verify your email,
                                or copy this to your browser
                                <br>{{ env('APP_URL') }}/user/verify/{{base64_encode($user->email)}}/{{base64_encode($user->verification_code)}}
                            @elseif($type == 2)
                                You have made a withdrawal of ${{ number_format($data) }}, kindly exercise some patience as the withdrawal is been processed.
                            @elseif($type == 3)
                                Your Deposit of ${{ number_format($data) }} has been received.
                            @elseif($type == 4)
                                Invoice for Deposit id {{ sprintf('%06d', $data) }} has expired, kindly create another invoice.
                            @elseif($type == 5)
                                Your wallet has been credited with  ${{ number_format($data) }} as investment profit, kindly check your dashboard to this effect.
                            @elseif($type == 6)
                                You have made a withdrawal of ${{ number_format($data) }} exclusing charges and it been approved, kindly check your dashbooard for transaction information
                            @else
                                Your wallet has been credited with ${{ number_format($data) }} as referral bonus, kindly check your dashboard to this effect.
                            @endif
